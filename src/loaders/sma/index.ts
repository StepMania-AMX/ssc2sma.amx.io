...
export default class SmaLoader {
      ...
      switch (tag) {
        ...
        case 'FGCHANGES':
          values = rawValues.map(
              (value: string[][]) =>
                  value.map((timingValues: string[]) => timingValues.join('='))
                      .join(',\r\n'));
      }
      ...
      for (const [tag, rawValues = []] of tags) {
        ...
        switch (tag) {
          ...
          case 'ATTACKS':
            values = rawValues.map(
                ({TIME, LEN, MODS}: any) =>
                    `TIME=${parseFloat(TIME).toFixed(3)}:LEN=${
                        parseFloat(LEN).toFixed(3)}:MODS=${MODS}`);
            break;

          case 'NOTES':
            values = rawValues.map(
                (value: any, i: number) => i === 5 ? value : `\t${value}`);
        }
        if (tag === 'DISPLAYBPM') {
  ...
  protected convertSscAttacks(attacks: any[]|null) {
    ...
    return attacks.filter((attack) => attack.MODS.every((mod: string) => {
      const parts = mod.split(' ');
      const modName = parts[parts.length - 1].toLowerCase()
      return smaModifiers.has(modName) ||
          /^(?:(?:c\d+)|(?:[\d\.]+x))$/.test(modName);
    }));
  }

  protected convertSscBgChanges(
      bgChanges: string[][][]|null, valueIfError: string[][][]|null = null) {
  ...
  protected convertSscCombos(
      combos: string[][][]|null, valueIfError: string[][][]|null = null) {
  ...
  protected convertSscNotes(
      sscLoader: SscLoader, sscStepTag: Map<string, any>) {
  ...
  protected convertSscScrolls(scrolls: string[][][]|null, numBeats: number) {
    ...
    for (const [beat, scroll] of scrolls[0]) {
      ...
      if (parseFloat(scroll) !== 1.0) {
        currentSpeedArea =
            [beat, scroll, scroll, (numBeats - parseFloat(beat)).toFixed(3)];
  ...
  protected convertSscSpeedChanges(
      speedChanges: string[][][]|null, valueIfError: string[][][]|null = null) {
  ...
  protected getDisplayBpm(
      tags: Map<string, any>, displayBpmFromSsc: string[][][]|null) {
  ...
  protected importFromSscStepTag(
    ...
    stepTag.set(
        'TICKCOUNT', sscLoader.getStepTagValue(sscStepTag, 'TICKCOUNTS'));
    stepTag.set(
        'SPEED',
    ...
    stepTag.set(
        ...
        this.convertSscScrolls(
            sscLoader.getStepTagValue(sscStepTag, 'SCROLLS'), numBeats));
    stepTag.set(
        'DISPLAYBPM',
        this.getDisplayBpm(
            stepTag, sscLoader.getStepTagValue(sscStepTag, 'DISPLAYBPM')));
    stepTag.set('METERTYPE', this.getSscMeterType(sscStepTag));
    stepTag.set(
        'ATTACKS',
        this.convertSscAttacks(
            sscLoader.getStepTagValue(sscStepTag, 'ATTACKS')));
    stepTag
        .set(
            'NOTES',
            this.convertSscNotes(sscLoader, sscStepTag),
            )
